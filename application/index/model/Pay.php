	 ...
	 */
	 public function isAutonym($uid){
		...
		public function validateCard($data){
			...
			return $this->wx_httrequest($url, $params );
		 ...
		 * 京东万象的SDK
		 */
		public function wx_httrequest($url, $params, $body="", $isPost=false, $isImage=false ) {
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_URL, $url."?".httbuild_query($params));
			if($isPost){
				if($isImage){
